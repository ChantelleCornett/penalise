`myredrank.iter` <- function(data, cols, colsR, fullcols, R,
                           clock, strata, Gamma.iter, print.level = 0)
{
  ...
  expr1 <- paste(expr1,", data=data, na.action=\"na.exclude\", control = coxph.control(timefix = FALSE))", sep = "")
  ...
  if (!is.null(fullcols)) expr2 <- paste(expr2,
                                         "+", paste(fullcovariates, collapse = "+"),sep="")
  expr2 <- paste(expr2,", data=data, na.action=\"na.exclude\", control = coxph.control(timefix = FALSE))", sep = "")
...
#' 
#' @export redrank
`myredrank` <- function(redrank, full = ~1, data, R,
                      strata = NULL, Gamma.start, method = "breslow", eps = 1e-5, print.level = 1)
{
  ...
  cx <- coxph(redrank, data=data, control = coxph.control(timefix = FALSE))
  ...
  while(abs(Delta) > eps) {
    ...
    ms.it <- myredrank.iter(data = rrdata, cols = cols, colsR = colsR,
